...
export const ENGLISH_UNITED_STATES: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "Untrusted Redirection",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "This problem is not caused by your account or device. For security reasons, the redirect link requested by you or the application you are using is considered untrustworthy by us. In this case, if you continue to log in, your account security may be threatened. To solve this problem, please try to return to the previous entrance and log in again.",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "The following content is confidential, please do not share it with anyone. If other people get the content, your account security will be threatened.",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "The above content is confidential, please do not share with anyone. If other people get the content, your account security will be threatened.",
};
...
export const RUSSIAN_RUSSIA: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "Ненадежное перенаправление",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "Эта проблема не вызвана вашей учетной записью или устройством. Из соображений безопасности ссылка на перенаправление, запрошенная вами или используемым вами приложением, считается ненадежной для нас. В этом случае, если вы продолжите входить в систему, безопасность вашей учетной записи может оказаться под угрозой. Чтобы решить эту проблему, попробуйте вернуться к предыдущему входу и войти снова.",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "Следующее содержание является конфиденциальным, пожалуйста, не делитесь им ни с кем. Если другие люди получат контент, безопасность вашей учетной записи будет поставлена ​​под угрозу.",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "Вышеуказанное содержание является конфиденциальным, пожалуйста, не делитесь с кем-либо. Если другие люди получат контент, безопасность вашей учетной записи будет поставлена ​​под угрозу.",
};
...
export const FRENCH_FRANCE: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "Redirection non approuvée",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "Ce problème n'est pas causé par votre compte ou votre appareil. Pour des raisons de sécurité, le lien de redirection demandé par vous ou l'application que vous utilisez est considéré comme non fiable par nous. Dans ce cas, si vous continuez à vous connecter, la sécurité de votre compte peut être menacée. Pour résoudre ce problème, veuillez essayer de revenir à l'entrée précédente et reconnectez-vous.",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "Le contenu suivant est confidentiel, veuillez ne le partager avec personne. Si d'autres personnes obtiennent le contenu, la sécurité de votre compte sera menacée.",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "Le contenu ci-dessus est confidentiel, veuillez ne le partager avec personne. Si d'autres personnes obtiennent le contenu, la sécurité de votre compte sera menacée.",
};
...
export const JAPANESE_JAPAN: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "信頼できないリダイレクト",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "この問題は、アカウントまたはデバイスが原因ではありません。 セキュリティ上の理由から、お客様またはお客様が使用しているアプリケーションによって要求されたリダイレクトリンクは、弊社では信頼できないと見なされます。 この場合、引き続きログインすると、アカウントのセキュリティが脅かされる可能性があります。 この問題を解決するには、前の入り口に戻ってもう一度ログインしてみてください。",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "次のコンテンツは機密情報です。誰とも共有しないでください。 他の人がコンテンツを入手すると、アカウントのセキュリティが脅かされます。",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "上記の内容は機密情報です。誰にも教えないでください。 他の人がコンテンツを入手すると、アカウントのセキュリティが脅かされます。",
};
...
export const KOREAN_KOREA: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "신뢰할 수없는 리디렉션",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "이 문제는 계정이나 장치로 인한 것이 아닙니다. 보안상의 이유로 귀하 또는 귀하가 사용하는 응용 프로그램에서 요청한 리디렉션 링크는 신뢰할 수없는 것으로 간주됩니다. 이 경우 계속 로그인하면 계정 보안이 위협받을 수 있습니다. 이 문제를 해결하려면 이전 입구로 돌아가서 다시 로그인하십시오.",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "다음 내용은 기밀이므로 다른 사람과 공유하지 마십시오. 다른 사람이 이러한 내용을 입수하면 계정 보안이 위협받을 수 있습니다.",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "위 내용은 기밀이므로 다른 사람과 공유하지 마십시오. 다른 사람이 이러한 내용을 입수하면 계정 보안이 위협받을 수 있습니다.",
};
...
export const CHINESE_TRADITIONAL: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "不可信任的重定向",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "這個問題不是由您的賬號或者設備引發的。為了安全考慮，您或者您正在使用的應用程序請求的重定向鏈接被我們認為是不可信任的。在這種情況下繼續登錄，您的賬號安全可能會收到威脅。解決這個問題，請嘗試回到之前的入口重新登錄。",
...
export const SPANISH_MEXICO: Record<PROFILE, string> = {
    ...
    [PROFILE.UNTRUSTED_REDIRECTION]: "Redirección no confiable",
    [PROFILE.UNTRUSTED_REDIRECTION_DESCRIPTION]: "Este problema no es causado por su cuenta o dispositivo. Por razones de seguridad, el enlace de redireccionamiento solicitado por usted o la aplicación que está utilizando no se considera confiable. En este caso, si continúa iniciando sesión, la seguridad de su cuenta puede verse amenazada. Para resolver este problema, intente regresar a la entrada anterior e inicie sesión nuevamente.",
    ...
    [PROFILE.DO_NOT_SHARE_CONTENT_BEFORE]: "El siguiente contenido es confidencial, no lo comparta con nadie. Si otras personas obtienen estos contenidos, la seguridad de su cuenta se verá amenazada.",
    [PROFILE.DO_NOT_SHARE_CONTENT_AFTER]: "El contenido anterior es confidencial, no lo comparta con nadie. Si otras personas obtienen estos contenidos, la seguridad de su cuenta se verá amenazada.",
};
